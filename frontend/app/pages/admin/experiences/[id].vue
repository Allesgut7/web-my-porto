...
import { emptyExperienceForm, experienceToForm } from '~/composables/useAdminExperiences'
...
const { getExperience, updateExperience } = useAdminExperiences()
...
const { data: experience, pending, error, refresh } = useAsyncData(
  `admin-experience-${id.value}`,
  () => getExperience(id.value),
  { watch: [id] },
)
...
watch(
  ...
  (value) => {
    if (value) {
      form.value = experienceToForm(value)
    }
  },
  { immediate: true },
)
...
useSeoMeta({
  title: () => experience.value?.title ? `Edit ${experience.value.title}` : 'Edit Experience',
  description: 'Edit pengalaman.',
})

onMounted(() => {
  if (route.query.created === '1') {
    successMessage.value = 'Pengalaman berhasil dibuat. Kamu sekarang bisa melengkapi atau mengedit detail.'
  }
})
...
async function handleSubmit() {
  ...
  try {
    const updated = await updateExperience(id.value, form.value)
    form.value = experienceToForm(updated)
    successMessage.value = 'Pengalaman berhasil diperbarui.'
    await refresh()
  } catch (error: any) {
    errorMessage.value =
      error?.data?.message ||
      error?.message ||
      'Gagal update pengalaman.'
  ...
  <div>
    <div class="mb-8">
      ...
      <h1 class="section-title">
        {{ experience?.title || 'Edit Experience' }}
      </h1>
      <p class="section-description">
        Update detail pengalaman.
      </p>
    </div>
